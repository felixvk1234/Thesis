...
class GraphSAGE(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, num_nodes, num_layers, dropout_rate=Config.DROPOUT_RATE):
        """
        GraphSAGE model with BatchNorm, ELU activation, and dropout.
        ...
        """
        super().__init__()
        print(f"[Checkpoint] Initializing GraphSAGE with {input_dim} input dim, "
        ...
        # GraphSAGE layers and BatchNorm
        self.convs = nn.ModuleList()
        self.bns = nn.ModuleList()

        for i in range(num_layers):
            conv = SAGEConv(hidden_dim, hidden_dim)
            nn.init.xavier_uniform_(conv.lin_l.weight, gain=0.1)
...
class Experiment:
    ...
    def run_hyperparameter_tuning(self):
                    ...
                    for alpha in Config.FOCAL_ALPHAS:
                        for gamma in Config.FOCAL_GAMMAS:
                            ...
                            try:
                                ...
                                trainer = GraphSAGETrainer(model, Config.DEVICE, alpha=alpha, gamma=gamma)
        ...
        if best_model is not None:
            ...
            _,_,_,best_alpha,best_gamma = best_config
            test_metrics = GraphSAGETrainer(best_model, Config.DEVICE, best_alpha, best_gamma).evaluate(self.data_test, calculate_emp=True)
